...
static void rockchip_dsi2_external_bridge_power_off(struct dw_mipi_dsi2 *dsi2)
{

	if (dm_gpio_is_valid(&dsi2->enable_gpio)) {
		dm_gpio_set_value(&dsi2->enable_gpio, 0);
	}
}


static void rockchip_dsi2_external_bridge_power_on(struct dw_mipi_dsi2 *dsi2)
{
	if (dm_gpio_is_valid(&dsi2->enable_gpio)) {
		dm_gpio_set_value(&dsi2->enable_gpio, 1);
		mdelay(1000);
	}
...
static void dw_mipi_dsi2_pre_enable(struct dw_mipi_dsi2 *dsi2)
{
	...
	dsi_write(dsi2, DSI2_PWR_UP, RESET);
	rockchip_dsi2_external_bridge_power_on(dsi2);
...
{
	struct dw_mipi_dsi2 *dsi2 = dev_get_priv(conn->dev);
	rockchip_dsi2_external_bridge_power_off(dsi2);
...
static int dw_mipi_dsi2_probe(struct udevice *dev)
{
	...
	printf("###%s: start to get tc358775 GPIO:\n", __func__);
	ret = gpio_request_by_name(dev, "enable-gpios", 0,
					   &dsi2->enable_gpio, GPIOD_IS_OUT);
	if (ret && ret != -ENOENT) {
	...
	printf("###%s: linux-uboot-successfullly to get tc358775 GPIO:\n", __func__);
