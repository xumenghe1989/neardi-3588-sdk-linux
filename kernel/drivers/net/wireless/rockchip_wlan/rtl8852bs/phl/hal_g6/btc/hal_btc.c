...
void _update_btc_state_map(struct btc_t *btc)
{
	...
	if (wl.connecting || wl._4way || wl.roaming) {
...
static u8 _update_bt_rssi_level(struct btc_t *btc, u8 rssi)
{
	...
	u8 *rssi_st = NULL, rssi_th = 0, j, rssi_level = 0;
			...
			u8 reason)
{
	...
	for (i = 0; i < ntfy_num; i++) {
		...
		if (linfo->role == PHL_RTYPE_STATION ||
			...
			diff_time = linfo_t->tx_time - last_tx_time;
			diff_time += (diff_time < 0 ? 0xffffffff : 0);

			diff_byte = linfo_t->tx_byte - last_tx_byte;
			diff_byte += (diff_byte < 0 ? 0xffffffffffffffff : 0);

			if (diff_time > 0)
				linfo_t->tx_tp = ((u32)_os_division64(diff_byte,diff_time)/125);

			diff_time = linfo_t->rx_time - last_rx_time;
			diff_time += (diff_time < 0 ? 0xffffffff : 0);

			diff_byte = linfo_t->rx_byte - last_rx_byte;
			diff_byte += (diff_byte < 0 ? 0xffffffffffffffff : 0);

			if (diff_time > 0)
				linfo_t->rx_tp = ((u32)_os_division64(diff_byte,diff_time)/125);
		}
...
void _btc_io_ofld (struct btc_t *btc, u32 srcpath, u32 addr, u32 mask, u32 value, bool trigger)
{
	struct rtw_hal_com_t *h = btc->hal;
	...
	src_grp = srcpath & 0x3;//0: BB, 1:RF, 2:MAC
	rf_path = srcpath >> 8;
