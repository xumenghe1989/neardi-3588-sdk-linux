			 ...
			 char *output, u32 *_out_len)
{
	...
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-25s = %d\n",
		 "TSSI Reset Count", tssi_info->high_pwr_rst_cnt);
			 ...
			 char *output, u32 *_out_len)
{
	...
	char *cmd[9] = {"-h", "rate", "limit", "limit_ru", "set_all", "set", "txshape", "constraint", "coex"};
	u32 used = *_used;
	u32 out_len = *_out_len;
	u32 val = 0;
	u32 tmp, tmp1, phy;
	...
	if (_os_strcmp(input[1], cmd[0]) == 0) {
		...
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "chalrf_wlan_tx_power_control   phy=%d   pwr_ctrl_idx=%d   tx_power_val=%d   enable=%d\n",
			 phy, tmp, tmp1, tmp2);
	} else
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
				 " No CMD find!!\n");
