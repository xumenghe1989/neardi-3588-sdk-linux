...
static int yt8521_led_init(struct phy_device *phydev)
{
	...
	*/
	...
	ret = ytphy_write_ext(phydev, YT8521_EXTREG_LED2, 0x1870);
	if (ret < 0)
		return ret;
	...
	//printk("yzhang..phy led1 , val=%#08x\n",val);
	if (val < 0)
		return val;
	...
	*/
	...
	ret = ytphy_write_ext(phydev, YT8521_EXTREG_LED1, 0x2600);
...
static int __maybe_unused yt8521_config_out_125m(struct phy_device *phydev)
{
#if 1
	int ret;
    int val;
	...
	printk("yzhang..phy 0x27, val=%#08x\n",val);
    if (ret < 0)
            return ret;
    ...
    val = ytphy_read_ext(phydev,  0xc);
	    printk("yzhang..phy 0x0c, val=%#08x\n",val);
    ...
    */
    val |= (3 << 1);
    ret = ytphy_write_ext(phydev, 0xa012, 0x68);
	
	val = ytphy_read_ext(phydev, 0xa012);
	printk("yzhang..phy 0xa012, val=%#08x\n",val);
	...
	//init sgmii->utp
		/*val = ytphy_read_ext(phydev, REG_PHY_SPEC_STATUS);
	if (val < 0)
		return val;

	val |= (1 << 5);
	ret = ytphy_write_ext(phydev, REG_PHY_SPEC_STATUS, val);
	if (ret < 0)
	    return ret;*/
...
static int  yt8521_config_init(struct phy_device *phydev)
{
	...
	ret = yt8521_config_out_125m(phydev);
