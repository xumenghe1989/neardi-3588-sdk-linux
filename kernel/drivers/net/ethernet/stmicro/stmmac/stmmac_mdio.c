...
// just For YT SMI Porting reference
struct mii_bus *new_bus;

EXPORT_SYMBOL(new_bus);

int yt_smi_cl22_write(u8 phyAddr, u8 regAddr, u16 regVal)
{

    if(!new_bus)
    {
        printk(KERN_ERR"%s g_mii_bus invalid!\n", __func__);
        return 0;
    }
    mutex_lock(&new_bus->mdio_lock);
    new_bus->write(new_bus, phyAddr, regAddr, regVal);
...
int yt_smi_cl22_read(u8 phyAddr, u8 regAddr, u16 *pRegVal)
{
    u16 regVal;
    ...
    mutex_lock(&new_bus->mdio_lock);
    regVal = new_bus->read(new_bus, phyAddr, regAddr);
    *pRegVal = regVal;
    mutex_unlock(&new_bus->mdio_lock);
