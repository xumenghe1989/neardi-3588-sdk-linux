...
static void __maybe_unused dsi_external_bradge_power_on(struct dw_mipi_dsi2 *dsi2)
{
	printk("...... dysun 0 %s\n",__FUNCTION__);
    ...
    if (dsi2->reset_gpio) {
		...
		gpiod_set_value(dsi2->reset_gpio, 1);
        usleep_range(1000, 2000);
        gpiod_direction_output(dsi2->reset_gpio, 0);
		gpiod_set_value(dsi2->reset_gpio, 0);
        usleep_range(1000, 2000);
        gpiod_direction_output(dsi2->reset_gpio, 1);
		gpiod_set_value(dsi2->reset_gpio, 1);
        usleep_range(1000, 2000);
...
static int dw_mipi_dsi2_probe(struct platform_device *pdev)
{
	...
	/*add for tc358775 pin control*/
		...
		if (IS_ERR(dsi2->reset_gpio)) {
			...
			DRM_DEV_ERROR(dev, "failed to request tc358775 reset GPIO: %d\n", ret);
			return ret;
		 }
