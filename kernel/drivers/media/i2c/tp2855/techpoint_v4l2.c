...
static __maybe_unused int techpoint_get_regulators(struct techpoint *techpoint)
{
	unsigned int i;
...
static int techpoint_analyze_dts(struct techpoint *techpoint)
{
	...
	if (ret)
		techpoint->xvclk_freq_value = 27000000;  //读取失败，默认设置

	ret = of_property_read_u32(node, TECHPOINT_CHANNEL_NUMS,
				   &techpoint->channel_nums);
	if (ret)
		techpoint->channel_nums = 4;  //读取失败，默认设置
...
static int techpoint_initialize_controls(struct techpoint *techpoint)
{
	...
	if (techpoint->input_type == TECHPOINT_DVP_BT1120) {
		...
		//debug
		dev_warn(dev, "initialize link_freq %lld\n",
			mode->link_freq_value);
		...
		__v4l2_ctrl_s_ctrl_int64(techpoint->pixel_rate_ctrl, pixel_rate);
		dev_dbg(dev, "initialize pixel_rate %lld\n", pixel_rate);
		//debug
		dev_warn(dev, "initialize pixel_rate %lld\n", pixel_rate);
	}
...
ret = clk_set_rate(techpoint->xvclk, TP2855_XVCLK_FREQ);
	if (ret < 0)
		dev_warn(dev, "Failed to set xvclk rate\n");
	if (clk_get_rate(techpoint->xvclk) != TP2855_XVCLK_FREQ)
			     ...
			     struct v4l2_subdev_format *fmt)
{
	struct techpoint *techpoint = to_techpoint(sd);
	const struct techpoint_video_modes *mode;
	dev_err(&techpoint->client->dev, "%s\n", __func__);

	mutex_lock(&techpoint->mutex);

	/*设置芯片的模式*/
	dev_err(&techpoint->client->dev, "start find best fmt\n");
	mode = techpoint_find_best_fit(techpoint, fmt);
	dev_err(&techpoint->client->dev, "find best fmt end\n");
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		dev_err(&client->dev, "%s start: get fmt%x %dx%d\n",
	...
	dev_err(&client->dev, "%s end: get fmt%x %dx%d\n",
		__func__, fmt->format.code,
		fmt->format.width, fmt->format.height);
				      ...
				      struct v4l2_subdev_frame_size_enum *fse)
{
	...
	dev_err(&techpoint->client->dev, "%s,index %d\n", __func__, fse->index);
...
//add
static int techpoint_get_mbus_config(struct v4l2_subdev *sd, unsigned int pad_id,
                                 struct v4l2_mbus_config *cfg)
{
	...
	if (techpoint->input_type == TECHPOINT_DVP_BT1120) {
		...
		cfg->flags = V4L2_MBUS_CSI2_4_LANE | V4L2_MBUS_CSI2_CHANNELS | V4L2_MBUS_CSI2_CONTINUOUS_CLOCK;
...
//add
static int techpoint_get_channel_info(struct techpoint *techpoint, struct rkmodule_channel_info *ch_info)
{
        if (ch_info->index < PAD0 || ch_info->index >= PAD_MAX)
		return -EINVAL;
        ch_info->vc = techpoint->cur_video_mode->vc[ch_info->index];
        ch_info->width = techpoint->cur_video_mode->width;
        ch_info->height = techpoint->cur_video_mode->height;
        ch_info->bus_fmt = techpoint->cur_video_mode->bus_fmt;
	dev_err(&techpoint->client->dev, "%s,W:%d, H:%d, FMT:%d\n", 		__func__, techpoint->cur_video_mode->width, techpoint->cur_video_mode->height, techpoint->cur_video_mode->bus_fmt);
        return 0;
}
//add
/* static int techpoint_get_bus_config(struct techpoint *techpoint, struct rkmodule_bus_config *bus_config)
{
	dev_err(&techpoint->client->dev, "%s\n", __func__);
	bus_config->bus.bus_type = V4L2_MBUS_CSI2_DPHY;
	bus_config->bus.lanes = V4L2_MBUS_CSI2_4_LANE | V4L2_MBUS_CSI2_CHANNELS | V4L2_MBUS_CSI2_CONTINUOUS_CLOCK;
	bus_config->bus.phy_mode = PHY_FULL_MODE;
       	return 0;
}*/
...
//	struct rkmodule_bus_config *bus_config;
	...
	dev_err(&techpoint->client->dev, "%s: cmd code : %u\n", __func__, cmd);

	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_MODULE_INFO cmd code : %lu\n", __func__, RKMODULE_GET_MODULE_INFO);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_AWB_CFG cmd code : %lu\n", __func__, RKMODULE_AWB_CFG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_AF_CFG cmd code : %lu\n", __func__, RKMODULE_AF_CFG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_LSC_CFG : %lu\n", __func__, RKMODULE_LSC_CFG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_HDR_CFG cmd code : %lu\n", __func__, RKMODULE_SET_HDR_CFG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_CONVERSION_GAIN cmd code : %lu\n", __func__, RKMODULE_SET_CONVERSION_GAIN);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_LVDS_CFG cmd code : %lu\n", __func__, RKMODULE_GET_LVDS_CFG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_DPCC_CFG cmd code : %lu\n", __func__, RKMODULE_SET_DPCC_CFG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_NR_SWITCH_THRESHOLD cmd code : %lu\n", __func__, RKMODULE_GET_NR_SWITCH_THRESHOLD);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_QUICK_STREAM cmd code : %lu\n", __func__, RKMODULE_SET_QUICK_STREAM);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_BT656_INTF_TYPE cmd code : %lu\n", __func__, RKMODULE_GET_BT656_INTF_TYPE);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_VC_FMT_INFO cmd code : %lu\n", __func__, RKMODULE_GET_VC_FMT_INFO);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_VC_HOTPLUG_INFO cmd code : %lu\n", __func__, RKMODULE_GET_VC_HOTPLUG_INFO);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_START_STREAM_SEQ cmd code : %lu\n", __func__, RKMODULE_GET_START_STREAM_SEQ);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_VICAP_RST_INFO cmd code : %lu\n", __func__, RKMODULE_GET_VICAP_RST_INFO);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_VICAP_RST_INFO cmd code : %lu\n", __func__, RKMODULE_SET_VICAP_RST_INFO);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_BT656_MBUS_INFO cmd code : %lu\n", __func__, RKMODULE_GET_BT656_MBUS_INFO);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_DCG_RATIO cmd code : %lu\n", __func__, RKMODULE_GET_DCG_RATIO);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_SONY_BRL cmd code : %lu\n", __func__, RKMODULE_GET_SONY_BRL);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_SYNC_MODE cmd code : %lu\n", __func__, RKMODULE_SET_SYNC_MODE);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_MCLK cmd code : %lu\n", __func__, RKMODULE_SET_MCLK);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_LINK_FREQ cmd code : %lu\n", __func__, RKMODULE_SET_LINK_FREQ);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_BUS_CONFIG cmd code : %lu\n", __func__, RKMODULE_SET_BUS_CONFIG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_GET_BUS_CONFIG cmd code : %lu\n", __func__, RKMODULE_GET_BUS_CONFIG);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_REGISTER cmd code : %lu\n", __func__, RKMODULE_SET_REGISTER);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SYNC_I2CDEV cmd code : %lu\n", __func__, RKMODULE_SYNC_I2CDEV);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SYNC_I2CDEV_COMPLETE cmd code : %lu\n", __func__, RKMODULE_SYNC_I2CDEV_COMPLETE);
	//dev_err(&techpoint->client->dev, "%s: RKMODULE_SET_DEV_INFO cmd code : %lu\n", __func__, RKMODULE_SET_DEV_INFO);


	switch (cmd) {
	case RKMODULE_GET_MODULE_INFO:
		dev_err(&techpoint->client->dev, "%s:get module info\n", __func__);
...
//		dev_err(&techpoint->client->dev, "%s:vc fmt\n", __func__);
//		if (!techpoint->streaming)
...
static const struct v4l2_subdev_pad_ops techpoint_subdev_pad_ops = {
	...
	//add
	.get_mbus_config = techpoint_get_mbus_config,
};
			   ...
			   const struct i2c_device_id *id)
{
	...
	sd = &techpoint->subdev;
	/*v4l2子设备初始化*/
	dev_info(dev, "v4l2 subdev init.\n");
	v4l2_i2c_subdev_init(sd, client, &techpoint_subdev_ops);

	/*解析设备树*/
	dev_info(dev, "dts analyze.\n");
	...
	/*初始化设备*/
	dev_info(dev, "initialize devices.\n");
	...
	/*初始化控制*/
	dev_info(dev, "initialize contronls.\n");
...
//	ret = media_entity_init(&sd->entity, PAD_MAX, techpoint->pad, 0);
	if (ret < 0)
		goto err_power_off;
#endif
