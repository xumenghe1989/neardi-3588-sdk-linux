...
static int check_chip_id(struct techpoint *techpoint)
{
	...
	techpoint_read_reg(client, 0x34, &chip_id);
...
//	static int flag = 0;
	...
	if (techpoint->power_on) {
		...
		} else if (techpoint->chip_id == CHIP_TP2855)
		{
			...
			//if(flag >= 10)
			//{
				dev_err(&client->dev, "start stream on \n");			
				tp2855_set_quick_stream(client, true);
				dev_err(&client->dev, "start stream end \n");
			//}
			//flag ++;			
		}
		up(&reg_sem);
				...
				struct rkmodule_vc_fmt_info *inf)
{
	...
	for (ch = 0; ch < PAD_MAX; ch++) {
		...
		dev_err(&client->dev, "%s: ch:%d get fps:%x %dx%d\n",
		__func__, ch, inf->fps[ch],
		inf->width[ch], inf->height[ch]);
	}
...
void techpoint_set_quick_stream(struct techpoint *techpoint, u32 stream)
{
	if (techpoint->chip_id == CHIP_TP2855) {
		dev_err(&techpoint->client->dev,
			"tp2855_set_quick_stream\n");
...
int techpoint_stop_video_stream(struct techpoint *techpoint)
{
	...
	if (techpoint->chip_id == CHIP_TP9930) {
		...
		tp2855_set_quick_stream(techpoint->client, false);
	}
	return 0;
}
