...
struct jaguar1 {
	...
	struct gpio_desc	*brst_gpio;
	struct gpio_desc	*brst2_gpio;
...
static int jaguar1_analyze_dts(struct jaguar1 *jaguar1)
{
	...
	jaguar1->brst_gpio = devm_gpiod_get(dev, "brst", GPIOD_OUT_LOW);
		if (IS_ERR(jaguar1->brst_gpio))
			dev_warn(dev, "can not find brst-gpios, error %ld\n",
				 PTR_ERR(jaguar1->brst_gpio));

	jaguar1->brst2_gpio = devm_gpiod_get(dev, "brst2", GPIOD_OUT_LOW);
		if (IS_ERR(jaguar1->brst2_gpio))
			dev_warn(dev, "can not find brst2-gpios, error %ld\n",
				 PTR_ERR(jaguar1->brst2_gpio));
			 ...
			 const struct i2c_device_id *id)
{
	...
	if(jaguar1->client->addr == 0x30){
		if (!IS_ERR(jaguar1->brst_gpio)) {
			gpiod_direction_output(jaguar1->brst_gpio, 0);
			usleep_range(1500, 2000);
			gpiod_direction_output(jaguar1->brst_gpio, 1);
		}

		if (!IS_ERR(jaguar1->brst2_gpio)) {
			gpiod_direction_output(jaguar1->brst2_gpio, 0);
			usleep_range(1500, 2000);
			gpiod_direction_output(jaguar1->brst2_gpio, 1);
		}
	}
