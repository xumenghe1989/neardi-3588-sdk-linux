...
static unsigned char previous_ahd_format;
...
static void  vd_pattern_enable(void)
{
	...
	gpio_i2c_write(0x62, 0x18, 0x13);
	gpio_i2c_write(0x62, 0x19, 0x13);
	gpio_i2c_write(0x62, 0x1a, 0x13);
	gpio_i2c_write(0x62, 0x1b, 0x13);
...
static void __maybe_unused jaguar1_video_cover_arb_scale_set(unsigned char devnum,unsigned int scalevalue)
{
	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0xFF, 0x20);
	usleep_range(25000, 30000);
	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0x01, scalevalue);
	usleep_range(25000, 30000);

}


static void jaguar1_video_set_channel_fomrat(unsigned char devnum,unsigned int ch,unsigned char index,unsigned char format,video_init_all *param)
{
	//bank0 ch=0x08/0x09/0x0a/0x0b
	unsigned char ahd_format=0xff;
	if(format==0xff)
		format=previous_ahd_format;

	switch(format){
		case 0x20:
		previous_ahd_format=0x20;
		ahd_format=0x0c;
		param->ch_param[index].format=AHD20_720P_30P_EX_Btype;
		jaguar1_video_cover_arb_scale_set(devnum,0x55);
		break;

		case 0x21:
		previous_ahd_format=0x21;
		ahd_format=0x0d;
		param->ch_param[index].format=AHD20_720P_25P_EX_Btype;
		jaguar1_video_cover_arb_scale_set(devnum,0x55);
		break;

		case 0x22:
		previous_ahd_format=0x22;
		ahd_format=0x04;
		param->ch_param[index].format=AHD20_720P_60P;
		jaguar1_video_cover_arb_scale_set(devnum,0x55);
		break;

		case 0x23:
		previous_ahd_format=0x23;
		ahd_format=0x05;
		param->ch_param[index].format=AHD20_720P_50P;
		jaguar1_video_cover_arb_scale_set(devnum,0x55);
		break;

		case 0x30:
		previous_ahd_format=0x30;
		ahd_format=0x02;
		param->ch_param[index].format=AHD20_1080P_30P;
		jaguar1_video_cover_arb_scale_set(devnum,0x00);
		break;

		case 0x31:
		previous_ahd_format=0x31;
		ahd_format=0x03;
		param->ch_param[index].format=AHD20_1080P_25P;
		jaguar1_video_cover_arb_scale_set(devnum,0x00);
		break;
	...
	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0xFF, 0x00);
	usleep_range(25000, 30000);
	gpio_i2c_write(jaguar1_i2c_addr[devnum], ch, ahd_format);
	usleep_range(25000, 30000);

	//printk(KERN_ERR".......dysun %s %d %x\n",__func__,__LINE__,format);
}

static unsigned char  jaguar1_video_get_channel_fomrat( unsigned char devnum, unsigned char ch)
{
	//bank1 ch=0x10/0x030/0x50/0x70
    unsigned char vfc_Value;
    gpio_i2c_write(jaguar1_i2c_addr[devnum], 0xFF, 0x01);
	usleep_range(25000, 30000);
	vfc_Value = gpio_i2c_read(jaguar1_i2c_addr[devnum], ch);
	//printk(KERN_ERR".......dysun %s %d %x\n",__func__,__LINE__,vfc_Value);
    return vfc_Value;
}


static void jaguar1_video_set_channel_auto_detect_mode( unsigned char devnum ,unsigned char bank)
{
	//bank5/6/7/8
	//printk(KERN_ERR".......dysun %s %d %x 0x05\n",__func__,__LINE__,jaguar1_i2c_addr[devnum]);
    gpio_i2c_write(jaguar1_i2c_addr[devnum], 0xFF, bank);
	usleep_range(25000, 30000);
	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0x82, 0xFF);  //After setting the normal channel mode, the value is 0x00
	usleep_range(25000, 30000);

	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0xFF, 0x13);
	usleep_range(25000, 30000);
	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0x30, 0xFF);
	usleep_range(25000, 30000);
	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0x31, 0xFF);
	usleep_range(25000, 30000);
	gpio_i2c_write(jaguar1_i2c_addr[devnum], 0x32, 0xFF);
	usleep_range(25000, 30000);
}
 ...
 *	warning			:
 *******************************************************************************/
extern int jaguar1_get_mipinum(void);

static void vd_set_all(video_init_all *param)
{
	int i,vfc_value[4];
	video_input_init  video_val[4];
	dev_num=jaguar1_get_mipinum();
	//printk(KERN_ERR".......dysun %s %d %d\n",__func__,__LINE__,dev_num);

	#if 1
	//int vfc_value[4];
	jaguar1_video_set_channel_auto_detect_mode(dev_num,0x05);
	vfc_value[0]=jaguar1_video_get_channel_fomrat(dev_num,0x10);
	jaguar1_video_set_channel_fomrat(dev_num,0x08,0x00,vfc_value[0],param);

	jaguar1_video_set_channel_auto_detect_mode(dev_num,0x06);
	vfc_value[1]=jaguar1_video_get_channel_fomrat(dev_num,0x30);
	jaguar1_video_set_channel_fomrat(dev_num,0x09,0x01,vfc_value[1],param);

	jaguar1_video_set_channel_auto_detect_mode(dev_num,0x07);
	vfc_value[2]=jaguar1_video_get_channel_fomrat(dev_num,0x50);
	jaguar1_video_set_channel_fomrat(dev_num,0x0a,0x02,vfc_value[2],param);

	jaguar1_video_set_channel_auto_detect_mode(dev_num,0x08);
	vfc_value[3]=jaguar1_video_get_channel_fomrat(dev_num,0x70);
	jaguar1_video_set_channel_fomrat(dev_num,0x0b,0x03,vfc_value[3],param);

	//printk(KERN_ERR".......dysun %s %d %x %x %x %x\n",__func__,__LINE__,vfc_value[0],vfc_value[1],vfc_value[2],vfc_value[3]);
#endif
	//ch_config=jaguar1_video_auto_vfc_get(dev_num,param);

#if 0
	for(i=0 ; i<4 ; i++)
	{
		printk(KERN_ERR"[DRV || %s] ch%d / fmt:%d / input:%d / interface:%d\n",__func__
				, param->ch_param[i].ch
	...
	jaguar1_video_set_channel_fomrat(dev_num,0x0a,0x02,vfc_value[2],param);
	jaguar1_video_set_channel_fomrat(dev_num,0x0b,0x03,vfc_value[3],param);
	//jaguar1_video_cover_arb_scale_set(dev_num,0x55);
...
for(i=0;i<2;i++){
	// Pad Control Setting
	gpio_i2c_write(jaguar1_i2c_addr[i], 0xff, 0x04);

	for(ii =0; ii<36; ii++)
	{
		gpio_i2c_write(jaguar1_i2c_addr[i], 0xa0 + ii , 0x24);
	}

	// Clock Delay Setting
	gpio_i2c_write(jaguar1_i2c_addr[i], 0xff, 0x01);
	for(ii =0; ii<4; ii++)
	{
		gpio_i2c_write(jaguar1_i2c_addr[i], 0xcc + ii , 0x64);
	}
#if 1
	// MIPI_V_REG_OFF
	gpio_i2c_write(jaguar1_i2c_addr[i], 0xff, 0x21);
	gpio_i2c_write(jaguar1_i2c_addr[i], 0x07, 0x80);
	gpio_i2c_write(jaguar1_i2c_addr[i], 0x07, 0x00);
#endif

#if 1
	// AGC_OFF  08.31
	gpio_i2c_write(jaguar1_i2c_addr[i], 0xff, 0x0A);
	gpio_i2c_write(jaguar1_i2c_addr[i], 0x77, 0x8F);
	gpio_i2c_write(jaguar1_i2c_addr[i], 0xF7, 0x8F);
	gpio_i2c_write(jaguar1_i2c_addr[i], 0xff, 0x0B);
	gpio_i2c_write(jaguar1_i2c_addr[i], 0x77, 0x8F);
	gpio_i2c_write(jaguar1_i2c_addr[i], 0xF7, 0x8F);
#endif
}
...
void jaguar1_stop(void)
{
	...
	arb_disable(dev_num);
	if(dev_num ==0){
		 gpio_i2c_write(0x60, 0xff, 0x20); // bank 0x20 stream control
		 // ARB RESET High
